...
title: Token 与概率
sidebar_position: 2
tags: [Token, next token prediction, LLM基础]
...
大语言模型不是像人一样直接“理解一整段文字后写出答案”，而是在每一步根据当前上下文预测下一个 token 的概率分布后按某种解码策略选出一个 token，再把它接回上下文，继续预测下一个 token。
