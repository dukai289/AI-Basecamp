...
大语言模型不是直接“理解一整段文字后写出答案”，而是在每一步根据当前上下文预测下一个 token 的概率分布，然后按某种解码策略选出一个 token，再把它接回上下文，继续预测下一个 token。

这个机制可以概括为一句话：

> LLM 的生成过程，是在 token 序列上反复做 next token prediction。

理解 token、logits、softmax 和概率分布，是理解上下文长度、输出随机性、temperature、top_p、计费、推理性能和模型幻觉的基础。

---

## 什么是 Token

Token 是模型处理文本的基本单位。它可以是一个汉字、一个英文单词、一个单词片段、一个标点、一个空格，甚至是某些特殊控制符。
...
模型不能直接处理字符串。Tokenizer 会把文本切成 token，再把每个 token 映射成一个整数 ID。

```text
"知识库" -> token -> token_id
```

词表就是 token 到 token id 的映射表。模型内部看到的是 token id 序列，而不是原始文字。
...
模型训练和推理时，都会围绕这些 token id 进行计算。

---

## 从 Token 到向量

Token id 进入模型后，会先被映射成向量，也就是 embedding。
...
Embedding 的作用是把离散的 token id 转成模型可以计算的连续向量。之后，Transformer 会结合上下文中所有 token 的信息，计算每个位置的隐藏状态。
...
Softmax 会把一组 logits 转成概率分布：

```text
prob(token_i) = exp(logit_i) / sum(exp(logit_j))
```
...
模型不是在脑子里保存一个确定答案，而是在每一步给出一个概率分布。解码器再根据这个分布选择下一个 token。

---

## 下一个 Token 预测

语言模型训练时，核心任务通常是 next token prediction。
...
模型一次前向计算通常只预测下一个 token。生成一段回答时，它会不断重复这个过程。
...
1. 用户输入文本。
2. Tokenizer 把文本切成 token ids。
3. 模型根据 token ids 计算下一个 token 的 logits。
...
优点是稳定、可复现、适合确定性任务。缺点是容易死板，可能陷入重复，也可能错过更自然的表达。
...
Sampling 更适合创作、头脑风暴、开放式问答；但对代码生成、数据抽取、结构化输出等任务，随机性过强会降低稳定性。

---

## Temperature 的直觉

Temperature 会改变概率分布的“尖锐程度”。
...
模型有最大上下文窗口，例如 32K、128K、1M tokens。输入、历史对话、工具结果、系统提示词和输出都要占用 token 预算。
...
输入越长，prefill 越重；输出越长，decode 轮数越多。

---

## Prompt 设计中的 Token 视角
...
对于结构化输出，最好配合 schema、parser、工具调用或 JSON mode，而不是只靠自然语言要求“请严格输出 JSON”。
...
Token 是模型处理文本的基本单位；logits 是模型对下一个 token 的原始打分；softmax 把 logits 转成概率分布；解码策略再从概率分布中选择 token。

理解这套机制后，很多 LLM 现象都会更容易解释：

- 为什么同一个问题可能多次回答不同。
- 为什么 temperature 和 top_p 会影响输出风格。
- 为什么长输出更慢、更贵。
- 为什么 prompt 会改变模型行为。
- 为什么概率高不等于事实正确。
