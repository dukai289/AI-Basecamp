...
模型每一步仍然是在上下文中预测下一个 token。提示工程关注的是：如何把任务、背景、约束、示例和输出格式组织成模型更容易执行的输入。
...
好的 prompt 不是“咒语”，而是任务接口。它让模型知道要做什么、依据什么、不能做什么、输出应该长什么样。

提示工程也要受上下文窗口限制。材料过多时，需要裁剪、摘要、检索或分阶段处理。
...
Agent 是由 LLM 驱动、能够基于目标选择动作、调用工具并根据反馈继续执行的任务系统。

和普通 Chatbot 相比，Agent 不只输出文本，还可能：
...
Agent 的核心不是“模型自己行动”，而是模型在工具、状态、权限、上下文和验证机制约束下，参与一个可控的执行循环。
...
8. 提示工程：理解如何把任务组织成模型可执行的上下文。
9. Agent：理解模型如何通过工具、状态和反馈执行任务。
10. 预训练：理解模型通用能力从哪里来。
11. 指令跟随：理解 base model 如何变成 chat model。
12. 微调：理解如何适配具体任务。
13. 强化学习：理解偏好对齐和推理训练。
14. 蒸馏：理解如何把强模型能力迁移到小模型。
15. 量化：理解如何降低部署成本。
16. 多模态模型：理解图像等输入如何接入 LLM。
17. 幻觉：理解生成模型的事实性风险。
18. 困惑度与损失函数：理解训练指标。
19. 质量与评估：理解如何判断模型好不好。
20. 模型性能：理解模型能不能稳定上线。
